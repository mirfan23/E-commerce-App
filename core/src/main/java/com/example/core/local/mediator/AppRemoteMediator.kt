...
class AppRemoteMediator(
    ...
    private val filter: DataFilter = DataFilter()
    ...
    override suspend fun load(
        ...
        return try {
            val responseData = apiEndPoint.fetchProduct(
                limitItem = state.config.initialLoadSize,
                page = page,
                search = filter.search,
                sort = filter.sort,
                brand = filter.brand,
                lowestPrice = filter.lowest,
                highestPrice = filter.highest
            )
            val endOfPaginationReached = responseData.data.totalPages <= page
